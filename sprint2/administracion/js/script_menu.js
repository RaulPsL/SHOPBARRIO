document.addEventListener("DOMContentLoaded", function() {
    ...
    const botonMenu=document.querySelector(".gestionar-menu");
    const ladoIzquierdo=document.querySelector(".lado-izquierdo");
    botonMenu.addEventListener('click', function(){
        if(menuActivo==false){
            ladoIzquierdo.classList.add("lado-izquierdo-mostrar");
            document.getElementsByClassName("gestionar-menu-icono")[0].classList.add("ocultar");
            document.getElementsByClassName("gestionar-menu-icono")[1].classList.remove("ocultar");
            menuActivo=true;
        }else{
            ladoIzquierdo.classList.remove("lado-izquierdo-mostrar");
            document.getElementsByClassName("gestionar-menu-icono")[0].classList.remove("ocultar");
            document.getElementsByClassName("gestionar-menu-icono")[1].classList.add("ocultar");
            menuActivo=false;
        }
    })
});
